//20052024 werkend maar alleen met strokepower geen sampletiming, dat gaat de hele tijd mis tijdens de whileloop 
...
void loop() {
    ...
    while (gyroX()  > 0) {  // stroke loop
      ...
      /*
      //if (millis()%500>100) {
      //Serial.println(sTime - postpowerTimeAccel);
      if ((sTime - postpowerTimeAccel) > sampleTime) {
        //postGyroX = gyroX();
        //accelXcalc = ((postGyroX - preGyroX)/ sampleTime);
        x = x + 1;
        Frower = ((outboardL/1000) * power() / (inboardL/1000)) * accelX();
        drivePower[x] = Frower;
...
float accelX() {
    ...
    accelXvalue = accel.x();
     // delay(5);
    accelXcalc = (outboardL/bnoPlacementL) * accelXvalue;
  return accelXcalc;
